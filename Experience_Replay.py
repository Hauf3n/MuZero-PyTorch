...
# save environment trajectories and sample sub trajectories
    ...
    # k = unroll | n = n-step-return | gamma = discount
        ...
        for step in range(start_index, start_index + k):
            ...
            if n_index >= traj_last_index: # end of episode
                ...
                v_n = torch.tensor([0]).to(device).to(dtype)#self.memory[memory_index]["vs"][n_index] * (gamma ** n) # discount v_n
            ...
            if step > 0  and step <= traj_last_index:
                ...
                sample["rewards"].append(torch.tensor([0]).to(device))
        ...
        last_valid_index = np.minimum(traj_last_index, start_index + k)
        num_steps = last_valid_index - start_index
        
        # real
        sample["actions"] = self.memory[memory_index]["actions"][start_index:start_index+num_steps]
       
        # fills
        num_fills = k - num_steps
